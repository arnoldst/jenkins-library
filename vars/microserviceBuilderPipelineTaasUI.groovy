    ...
    node('microclimatePod') {
      ...
      if (build) {
        if (fileExists('pom.xml')) {
          stage ('Maven Build') {
            container ('maven') {
              sh "cd lib && mvn install:install-file -Dfile=developer-dashboard-dao-0.16.0-SNAPSHOT.jar -DgroupId=systems-reliability-tools -DartifactId=developer-dashboard-dao -Dversion=0.16.0-SNAPSHOT -Dpackaging=jar"
